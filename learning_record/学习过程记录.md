...
https://github.com/qbdl/3D_RPG_learning 仓库的开发学习记录.
...
- **2.6 增加公主NPC &  四大护卫**：与巫师贤者具有羁绊，会有关联对话
...
- 如果想要NPC不被人物撞飞，就可以不加入rigidbody,但如果又想要能够不陷进地里
  - 解决方案：在人物prefab里面创建一个空物体，位置就置于人物脚底，这样在选中移动模式后，移动时，按住V（按住右键去找那个点）可以令整个人物的该点去吸附顶点。

- .meta的用途：

  | **GUID 写在 .meta 里**     | Unity 的引用关系（Prefab → 材质、场景 → 预制体…）都存储的是 **GUID**。当你在 Project 视图里移动、重命名文件时，Unity 只是在数据库里把 *路径 ←→ GUID* 的映射改了一下。GUID 本身没变，其他资源依旧能通过 GUID 找到它。 |
  | -------------------------- | ------------------------------------------------------------ |
  | **.meta 必须随文件一起动** | 如果你移动的只是 *.prefab*，却忘了它旁边的 *.prefab.meta*，Unity 会给新位置的文件重新生成一个 **新的 GUID**，**这时原来的引用就“断线”了。** |

  - **结论：**

    > *在同一个工程里，只要「文件 + 对应 .meta」一起拖/剪切，路径怎么改都不影响引用。*

- 尽量避免Unity与VSCode同时修改，有的时候会出现cache的错误，导致文件更改与想象不符。

- 当**只是使用素材包里的一部分时，要去掉其他不必要的内容**却不知道哪些有用时：

  1. 在 **Project** 里多选需要的 Prefab → **右键 ▸ Select Dependencies**
  2. 点击 *Project* 右上 **🔒 Padlock** 锁定当前高亮集合
  3. 搜索框里输入： `t:texture ref:selection` 并敲 Enter /或者直接看也行（删去没有被选中的内容即可
...
- **3.2 狼人**
  ...
  - 挂载的代码将EnemyController.cs换成Grunt.cs——用于使用击飞
...
实现流程：**主要加在TakeDamage里 & 动画触发**

1. 玩家按下防御键，进入防御状态`isGuard`和 `characterStats.isDefend`。
2. 当敌人攻击玩家时，调用`TakeDamage` 方法。
3. 如果玩家处于防御状态，进入盾反逻辑：
   - 计算反弹伤害并应用到攻击者。
   - 触发攻击者的受击动画。
4. 动画系统同步防御状态，确保视觉效果。
...
- **CharacterStats.cs** 的 两个TakeDamage中的第一个
...
- **PlayereController.cs** 代码修改，增加对应的变量，以及动画对应，同时设置长按G键启动盾反
...
| 竞技场          | 和主线同装备              | 难度对应“≈”注解                | 依各怪特性调优：骨头怪偏脆，狼人攻速快，雪人近战肉搏，黑牛将军终极考验 |
...
| **飞行兽**               | 90         | 3            | 40         | 1.5          | 3.0         | 1.0      | 12      | 18      | 2.2       | 0.25        |
...
| **雪人**     | 110        | 5            | 45         | 1.7          | 2.5         | 2.0      | 8       | 12      | 1.8       | 0.25        | 近战怪                    |
| **黑牛将军** | 250        | 10           | 100        | 2.8          | 3.8         | 1.5      | 15      | 25      | 2.5       | 0.30        | 终极 Boss，冲锋技能附晕眩 |
...
在CharacterStats.cs里增加对应回血参数，在它的Update里实现回血。
